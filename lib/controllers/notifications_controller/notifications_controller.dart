...
class NotificationsController extends GetxController {
  ...
  Future<void> _loadPreferences() async {
    try {
      ...
      final onboardingTime = prefs.getString(
        'reminderTime',
      ...
      if (onboardingTime != null && onboardingTime.isNotEmpty) {
        ...
        _logger.i('✅ Loaded reminder time from onboarding: $onboardingTime');
      } else {
        // Fallback: check if they've set it in notifications page before
        scheduleTime.value =
            prefs.getString('schedule_time') ?? 'Every day at 7:00 PM';
        _logger.i('ℹ️ Using default or previously saved time');
      }
  ...
  Future<void> openSchedulePicker() async {
    ...
    try {
      ...
      _logger.w('⚠️ Could not parse time, using default: $e');
    }
  ...
  void _scheduleDailyReminder() {
    ...
    final timeOnly = scheduleTime.value.replaceAll('Every day at ', '');
    _logger.i('📅 Daily reminder scheduled at $timeOnly');
