...
class UnifiedPlayerController extends GetxController {
  ...
  final RxInt _selectedTabIndex = 0.obs; // 0 = Learn (Video), 1 = Meditate (Audio)
  ...
  String get audioRemainingText => _formatDuration(audioDuration - audioPosition);
      ...
      if (args.containsKey('session')) {
        ...
        if (_session.value?.videoUrl != null) {
          loadVideo(
            ...
            itemId: isDownloaded ? 'course_session_${_session.value!.id}' : null,
          );
        }
  ...
  Future<void> loadVideo(String videoUrl, {bool isDownloaded = false, String? itemId}) async {
    try {
      ...
      if (isDownloaded && itemId != null) {
        ...
        if (localPath != null && await File(localPath).exists()) {
          ...
          _videoController = video_player.VideoPlayerController.file(File(localPath));
        } else {
        ...
        _videoController = video_player.VideoPlayerController.networkUrl(Uri.parse(videoUrl));
      }
      ...
      Get.snackbar('Error', 'Failed to load video: $e', snackPosition: SnackPosition.BOTTOM);
    } finally {
  ...
  Future<void> loadAudio(String audioUrl) async {
    try {
      ...
      if (duration != null) {
        ...
        _logger.i('✅ Audio loaded successfully - Duration: ${_formatDuration(duration)}');
      } else {
