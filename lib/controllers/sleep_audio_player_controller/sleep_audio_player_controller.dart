...
class SleepAudioPlayerController extends GetxController {
  ...
  String get formattedTime {
    final remaining = (_totalDuration.value - _elapsedSeconds.value).clamp(0.0, _totalDuration.value);
  ...
  Future<void> _loadSleepData() async {
    try {
      ...
      _logger.i('💤 Loading sleep audio: ${_sleep.value.title} for ${_durationMinutes.value} min');
  ...
  Future<void> _loadAudio() async {
    try {
      ...
      _logger.i('📏 Actual audio duration: ${_actualAudioDuration?.inSeconds}s');
      ...
      _logger.i('✅ Audio loaded successfully, will play for: ${_totalDuration.value}s');
    ...
    _elapsedTimer = Timer.periodic(const Duration(milliseconds: 100), (timer) {
      if (_isPlaying.value && _elapsedSeconds.value < _totalDuration.value) {
        ...
        if (_elapsedSeconds.value >= _totalDuration.value) {
          ...
          _elapsedTimer?.cancel();
          _logger.i('⏸️ Stopped at selected duration: ${_totalDuration.value}s');
        }
      }
    });
  }
  ...
  void skipBackward() {
    try {
      final newElapsed = (_elapsedSeconds.value - 15).clamp(0.0, _totalDuration.value);
      ...
      if (_actualAudioDuration != null) {
        ...
        _logger.i('⏪ Skipped backward - Elapsed: ${newElapsed}s, Audio pos: ${audioPosition}s');
      }
    } catch (e) {
  ...
  void skipForward() {
    try {
      ...
      if (_actualAudioDuration != null) {
        ...
        final audioPosition = newElapsed % audioDurationSeconds;
        _audioPlayer.seek(Duration(seconds: audioPosition.toInt()));
        _logger.i('⏩ Skipped forward - Elapsed: ${newElapsed}s, Audio pos: ${audioPosition}s');
      }
    } catch (e) {
  ...
  void seekTo(double value) {
    try {
      ...
      if (_actualAudioDuration != null) {
        ...
        final audioPosition = newElapsed % audioDurationSeconds;
        _audioPlayer.seek(Duration(seconds: audioPosition.toInt()));
        _logger.i('🎯 Seeked - Elapsed: ${newElapsed}s, Audio pos: ${audioPosition}s');
      }
    } catch (e) {
